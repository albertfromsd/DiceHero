...
def dice_hero(request):
    ...
    ### [ENEMY] ###
    #after enemy created in database

### [ATTACK VALUES] from hero_roll / enemey_roll ###
...
# NOT for the number of sides on a die
    ...
    if "hero_life" not in request.session:
        ...
        request.session["hero_life"] = hero_life

    #### Retrieve Enemy ####
    enemy_hpb = 100
    if "enemy_life" not in request.session:
        request.session["enemy_life"]=enemy_hpb
        enemy_life = request.session["enemy_life"]
    else:
        enemy_life = request.session["enemy_life"]
        enemy_life-=hero_attack
        request.session["enemy_life"] = enemy_life
...
def god_wpn_dice(request):
    ...
    #[CONTEXT] DICE SIDE VARs FROM SESSION (saved in /process_god_wpn_dice)
    ...
    return render(request, "god_wpn_dice.html", context)

def del_wpn_dice(request):
    dice_to_delete=request.POST["delete"]
    WpnDice.objects.get(id=dice_to_delete).delete()

    return redirect("/god_wpn_dice")

def edit_wpn_dice(request):
    if "edit_name" in request.POST:
...
def process_god_wpn_dice(request):

...
def process_create_hero(request):
    ...
    ###############################################
    #### DIFFERENCE BETWEEN THE ID AND THE ACTUAL ISNTANCE, SOMETIMES NEED ONE OR THE OTHER
    ################################################
...
# NOT for the number of sides on a die
    ...
    for i in range(0, len(enemy_dice), 1):
        ...
        request.session["enemy_roll"].insert(i, enemy_dice[num])
...
# NOT for the number of sides on a die
    ...
    for i in range(0, len(hero_dice), 1):
        if len(hero_dice[i])==0:
            continue
        num=random.randint(0, len(hero_dice[i]))
...
def edit_equip(request):
    ...
    if "edit_owner" in request.POST:
        ...
        new_owner.add_owner=Hero.objects.get(id="3")
        new_owner.save()
